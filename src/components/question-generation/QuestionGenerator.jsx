...
const QuestionGeneration = () => {
  ...
  const questionGenModule = useSelector(state => state?.quesGen?.moduleMCQs)
  ...
  const getQuestionRange = (currentIndex) => {
    ...
    const end = Math.min(start + itemsPerPage, questionGenModule.length); // Calculate the end index
    return { start, end };
  };
  ...
  const toggleAccordion = (index) => {
    setIsAccordionOpen((prev) => {
      if (Array.isArray(prev)) {
        ...
        return new Array(questionGenModule.length).fill(false);
      }
    });
  };
  ...
  const handleCheckAnswer = () => {
    ...
    if (selectedAnswer) {
      ...
      const correctAnswer = JSON.parse(questionGenModule[currentIndex].answers)[questionGenModule[currentIndex].correct_Answer];
  ...
  const nextPage = () => {
    if ((currentPage + 1) * itemsPerPage < questionGenModule.length) {
  ...
  // questionGenModule[currentIndex].explanationList.map((explanation, index) => {
  //   let isSelected = selectedAnswer === explanation;
  //   const isCorrectAnswer = index === questionGenModule[currentIndex].correctAnswerId;
  // })
  ...
  useEffect(() => {
    ...
    if (questionGenModule.length === currentIndex + 1) {
      setIsReviewEnabled(true); // Enable the Finish button when the condition is met 
    }

  }, [currentIndex, questionGenModule.length]); // Re-run whenever currentIndex changes
  ...
  useEffect(() => {
    const handleKeyPress = (e) => {
      ...
      if (keyIndex !== -1 && keyIndex < questionGenModule[currentIndex]?.answersArray.length) {
        const answer = questionGenModule[currentIndex].answersArray[keyIndex];
  ...
  }, [currentIndex, attempted, questionGenModule]);
      ...
      <div className="dark:bg-[#1E1E2A] min-h-screen">
        ...
        <div className=" mx-auto  flex items-center justify-center p-6  text-black   ">
          ...
          <div className={`max-w-full  transition-all duration-150  w-[100%] md:w-[80%] lg:w-[70%] xl:w-[55%] ${toggleSidebar ? "2xl:w-[55%] " : '2xl:w-[40%] '} ${toggleSidebar ? "lg:mr-[130px] " : 'lg:mr-[200px] '}  `}>
            ...
            <div className="bg-[#3CC8A1]   text-white p-6 rounded-md flex items-center justify-between relative">
              ...
              <div className="absolute bottom-0 left-0 w-full h-[4px]  bg-[#D4D4D8] rounded-md overflow-hidden">
                <div
                  ...
                  style={{ width: `${((currentIndex + 1) / questionGenModule.length) * 100}%` }}
                ></div>
              </div>
              ...
              <div className="flex items-center space-x-4 absolute left-1/2 transform -translate-x-1/2 text-[14px] lg:text-[18px]">
                ...
                <h2 className="font-semibold text-center">
                  Question {currentIndex + 1} of {questionGenModule.length}
                </h2>
                <button className={`text-white ${currentIndex + 1 === questionGenModule.length ? 'opacity-70 cursor-not-allowed' : ''}`} onClick={nextQuestion}>
                  &rarr;
                </button>
              </div>
            ...
            {/* Question start */}
            {questionGenModule.length > 0 && (
              <div className="mt-6 p-6" key={currentIndex}>
                <p className="text-[#000000] text-[14px] text-justify lg:text-[16px] dark:text-white">
                  {questionGenModule[currentIndex].question_stem}
                </p>

                <h3 className="mt-4 text-[12px] lg:text-[14px] text-[#3F3F46] font-bold dark:text-white">
                  {questionGenModule[currentIndex].lead_in_question}
                </h3>

                {/* Options Section */}
                <div className="mt-4 space-y-4">
                  {JSON.parse(questionGenModule[currentIndex].answers).map((answer, index) => {
                    const isSelected = selectedAnswer === answer;
                    const isCorrectAnswer = index === questionGenModule[currentIndex].correct_Answer;

                    // Determine the border and background colors
                    const borderColor = isButtonClicked || attempted[currentIndex] !== null
                      ? isCorrectAnswer ? "border-[#22C55E]" : "border-[#EF4444]"
                      : "";
                    const bgColor = isButtonClicked || attempted[currentIndex] !== null
                      ? isCorrectAnswer ? "bg-[#DCFCE7]" : "bg-[#FEE2E2]"
                      : "";

                    // Parse explanations
                    let explanations;
                    try {
                      explanations = JSON.parse(questionGenModule[currentIndex].explanation);
                    } catch (error) {
                      ...
                      <div key={index}>
                        {!isAccordionVisible && attempted[currentIndex] === null ? (
                          ...
                          >
                            ...
                            {isAccordionOpen[index] && explanations[index] && (
                              <>
          ...
          <div className={`hidden lg:block fixed right-0 top-0`}>
            <div className={`absolute right-0 top-0 bg-white w-[28%] md:w-[25%] lg:w-[240px] dark:border-[1px] dark:border-[#3A3A48] h-screen dark:bg-[#1E1E2A] text-black ${!toggleSidebar ? "translate-x-0" : "translate-x-full"} transition-transform duration-300`}>
              ...
              <div className="flex items-center justify-center gap-x-28 mt-3 text-[#71717A]">
                ...
                <button
                  className={`${((currentPage + 1) * itemsPerPage) >= questionGenModule.length ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                  onClick={((currentPage + 1) * itemsPerPage) < questionGenModule.length ? nextPage : null}
                  disabled={((currentPage + 1) * itemsPerPage) >= questionGenModule.length}
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="24"
