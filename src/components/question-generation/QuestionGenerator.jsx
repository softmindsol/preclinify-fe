...
const QuestionGeneration = () => {
  ...
  const handleCheckAnswer = () => {
    ...
    if (selectedAnswer) {
      ...
      const correctAnswer = JSON.parse(questionGenModule[currentIndex].answers)[
        questionGenModule[currentIndex].correct_Answer
      ];
    ...
    if (currentIndex < questionGenModule.length - 1) {
      ...
      if (isQuestionReview) {
        ...
        if (questionGenModule[currentIndex]?.conditionName !== null) {
          dispatch(
            fetchConditionNameById({ id: questionGenModule[currentIndex]?.conditionName })
          )
            .unwrap()
            .then(res => {
              setArticle(res);
  ...
  useEffect(() => {
    const handleKeyPress = e => {
      ...
      if (
        keyIndex !== -1 &&
        keyIndex < questionGenModule[currentIndex]?.answersArray.length
      ) {
            ...
            <div className='bg-[#3CC8A1]   text-white p-6 rounded-md flex items-center justify-between relative'>
              ...
              <div className='absolute bottom-0 left-0 w-full h-[4px]  bg-[#D4D4D8] rounded-md overflow-hidden'>
                <div
                  ...
                  style={{
                    width: `${((currentIndex + 1) / questionGenModule.length) * 100}%`,
                  }}
                ></div>
              </div>
              ...
              <div className='flex items-center space-x-4 absolute left-1/2 transform -translate-x-1/2 text-[14px] lg:text-[18px]'>
                ...
                <button
                  className={`text-white ${
                    currentIndex + 1 === questionGenModule.length
            ...
            {questionGenModule.length > 0 && (
              <div className='mt-6 p-6' key={currentIndex}>
                ...
                <div className='mt-4 space-y-4'>
                  {JSON.parse(questionGenModule[currentIndex].answers).map(
                    (answer, index) => {
                      const isSelected = selectedAnswer === answer;
                      const isCorrectAnswer =
                        index === questionGenModule[currentIndex].correct_Answer;
                      ...
                      try {
                        explanations = JSON.parse(
                          questionGenModule[currentIndex].explanation
                        );
                      } catch (error) {
            ...
            >
              <div className='w-full'>
                              ...
                              {flaggedQuestions[num] ? (
                                ...
                                >
                                  <svg
                                    ...
                                    stroke={
                                      flaggedQuestions[num] ? 'white' : 'currentColor'
                                    }
                                    strokeWidth='2'
                                    strokeLinecap='round'
                                    strokeLinejoin='round'
                ...
                <div className='flex items-center justify-center gap-x-28 mt-3 text-[#71717A]'>
                  ...
                  <button
                    className={`${
                      (currentPage + 1) * itemsPerPage >= questionGenModule.length
                        ? 'opacity-50 cursor-not-allowed'
                        : 'cursor-pointer'
                    }`}
                    onClick={
