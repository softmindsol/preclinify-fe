...
const SbaPresentation = () => {
  ...
  const presentationData = useSelector(
    (state) => state.presentations.mcqs || [],
  );
  ...
  const filteredItems = presentationData
    ?.slice(start, end)
  ...
  const handleCheckAnswer = () => {
    ...
    if (selectedAnswer) {
      ...
      const correctAnswer =
        presentationData[currentIndex].answersArray[
          presentationData[currentIndex].correctAnswerId
        ];
        ...
        if (presentationData[currentIndex]?.conditionName !== null) {
          dispatch(
            fetchConditionNameById({
              id: presentationData[currentIndex]?.conditionName,
            }),
          )
            .unwrap()
            .then((res) => {
  ...
  useEffect(() => {
    if (presentationData?.length) {
      const initialAccordionState = presentationData[
        currentIndex
            ...
            <div className="relative flex items-center justify-between rounded-md bg-[#3CC8A1] p-6 text-white">
              ...
              <div className="absolute bottom-0 left-0 h-[4px] w-full overflow-hidden rounded-md bg-[#D4D4D8]">
                <div
                  ...
                  style={{
                    width: `${((currentIndex + 1) / presentationData?.length) * 100}%`,
                  }}
                ></div>
              </div>
            ...
            {presentationData.length > 0 && (
              <div className="mt-6 p-6" key={currentIndex}>
                ...
                <div className="mt-4 space-y-4">
                  {presentationData[currentIndex]?.answersArray.map(
                    (answer, index) => {
