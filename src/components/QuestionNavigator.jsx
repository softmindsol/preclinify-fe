...
const QuestionNavigator = ({ attempted, flaggedQuestions, visited, setCurrentIndex }) => {
    ...
    // Function to filter questions based on selection
    const filteredQuestions = useMemo(() => {
        ...
        switch (selectedFilter) {
            case "Flagged":
                return validNumbers(flaggedQuestions).filter((key) => flaggedQuestions[key] === true);
            case "Unseen":
            ...
            default:
                return Array.from(
                    new Set([
                        ...
                        ...validNumbers(flaggedQuestions).filter((key) => flaggedQuestions[key] === true),
                        ...validNumbers(visited).filter((key) => visited[key] === true || attempted[key] === false)
                    ])
                ).sort((a, b) => a - b);
        }
    ...
    const handleFilterChange = (filter) => {
        ...
        setPagination((prev) => ({
            ...prev,
            [filter]: 0 // Reset pagination when switching filters
        }));
    };
        ...
        <div>
            ...
            <div className="flex items-center justify-between p-5 w-full text-[12px] dark:text-white">
                {["All", "Flagged", "Unseen"].map((filter) => (
                    <span
                        key={filter}
                        className={`w-[30%] text-center cursor-pointer ${selectedFilter === filter ? 'text-[#3CC8A1] border-b-[1px] border-[#3CC8A1]' : 'hover:text-[#3CC8A1]'}`}
            ...
            <div className="flex justify-around gap-3 mt-4">
                ...
                <button
                    ...
                    disabled={(pagination[selectedFilter] + 1) * pageSize >= filteredQuestions.length}
                >
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="24"
